	...
	/** GENERATE individual event data	for event list array */
		public function generate_event_data(
			...
			// Initial variables
				...
				$login_needed_message = sprintf("%s <a href='%s' class='evcal_btn'>%s</a>", evo_lang('Login required to see the information') , wp_login_url($current_url), evo_lang('Login'));
				...
				*/
				...
				// EACH DATA FIELD
					...
					// EVENT LOCATION
						...
						$_eventcard['timelocation'] = array(
							...
							'address'=> ($hide_location_info ? $login_needed_message : $location_address),
							...
							if( !empty($this->evopt1['evcal_ec_f'.$x.'a1']) && !empty($this->evopt1['evcal__fai_00c'.$x])	&& !empty($ev_vals["_evcal_ec_f".$x."a1_cus"])	){
								...
								if(empty($this->evopt1['evcal_ec_f'.$x.'a3']) || $this->evopt1['evcal_ec_f'.$x.'a3']=='no'){
									...
									$_eventcard['customfield'.$x] = array(
										...
										'login_needed_message'=> ( (evo_settings_check_yn( $this->evopt1 , 'evcal_ec_f'.$x.'a5') && !$is_user_logged_in && $visibility_type=='loggedin')? $login_needed_message: '' ),
				...
				// SCHEME SEO
					...
					if(!$show_schema){
						...
						$__scheme_data ='<a href="'.$event_permalink.'"></a></div>';
						$__scheme_attributes = '';
					}else{
