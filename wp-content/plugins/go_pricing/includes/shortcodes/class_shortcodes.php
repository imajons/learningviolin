...
class GW_GoPricing_Shortcodes {
	...
	public function pricing_table_sc( $atts, $content = null ) {
		...
		$main_head_html .= sprintf( 
			...
			!empty( $pricing_table['col-width']['max'] ) && (int)$pricing_table['col-width']['max'] > 0  ? ' data-mw="' . esc_attr( (int)$pricing_table['col-width']['max'] ) . '"' : ''
		);
		...
		$html = GW_GoPricing_Helper::parse_shortcodes( $html );
